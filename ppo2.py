...
class PPO2(ActorCriticRLModel):
    ...
    def __init__(self, policy, env, gamma=0.99, n_steps=128, ent_coef=0.01, learning_rate=2.5e-5, vf_coef=0.5,
...
class Runner(AbstractEnvRunner):
    ...
    def run(self):
        ...
        for steps in range(self.n_steps):
            ...
            xpos = int(infos[0]['screen_x'])
            
            if xpos == 0:
                xpos_old = 0
                
            rings = int(infos[0]['rings'])
            
            if rings == 0:
                rings_old = 0
                
            try:
                xpos_old = xpos - (xpos - xpos_old)
            except:
                xpos_old = xpos
                
            try:
                rings_old = rings -(rings - rings_old)
            except:
                rings_old = rings
            
            progress_eval += xpos - xpos_old
            
            if steps % 32 == 0:
                #print("xpos: ", xpos, "xpos_old: ", xpos_old)
                pass
            ...
            if steps == 127:
               # print("progress_eval: ", progress_eval)
                if progress_eval < 0:
                    rewards -= 100
                    self.env.reset()
                    #print("reset")
